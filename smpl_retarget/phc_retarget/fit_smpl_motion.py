...
def main(cfg: DictConfig) -> None:
    all_pkls = glob.glob("../taihu/run/*.npz", recursive=True)
    ...
    for key in key_names:
        data = process_motion([key], key_name_to_pkls, cfg)
        all_data.update(data)

    # import ipdb; ipdb.set_trace()
    if len(all_data) == 1:
        data_key = list(all_data.keys())[0]
        os.makedirs(f"./retargeted_motion_data/phc/taihu", exist_ok=True)
        dumped_file = f"./retargeted_motion_data/phc/taihu/{data_key}_origin.pkl"
        print(dumped_file)
        ...
        for data_key, data_value in all_data.items():
            dumped_file = f"./retargeted_motion_data/phc/taihu/{data_key}_origin.pkl"
            print(dumped_file)
            # pickle.dump({data_key: data_value}, dumped_file)
            import ipdb; ipdb.set_trace()
            joblib.dump({data_key: data_value}, dumped_file)
